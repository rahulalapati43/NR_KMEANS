...
def clustering(dataset,clusters_input):
    ...
    m = features_array.shape[1]/nrOfSubspaces
    ...
    for k in range(0, nrOfSubspaces):
        if (k == nrOfSubspaces - 1):
            endOfRange = features_array.shape[1]
        else:
    ...
    for subspace in range(0,nrOfSubspaces):
        ...
        kmeans = KMeans(n_clusters=nrOfclusters, init="k-means++").fit(features_array)
        ...
        for run in range(0,m):
            ...
            randVt = ortho_group.rvs(features_array.shape[1])
            ...
            X = np.matmul(features_array,dpProj)
